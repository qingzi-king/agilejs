...
export const MessageBubble: React.FC<{ message: ChatMessage }> = ({ message }) => {
  ...
  if (message.status === 'pending') {
    return (
      <div className="flex justify-start">
        <div className="max-w-[85%] rounded-lg px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-sm">
  ...
  if (message.status === 'thinking') {
    return (
      <div className="flex justify-start">
        <div className="max-w-[85%] rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700">
          {/* 思考状态头部 */}
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        {showThinking && message.thinkingContent && (
          <div className="flex justify-start">
            <div className="max-w-[85%] rounded-lg px-3 py-2 bg-purple-50 dark:bg-purple-900/20 text-xs text-gray-500 dark:text-gray-400 max-h-32 overflow-y-auto whitespace-pre-wrap">
              {message.thinkingContent}
            </div>
          </div>
        )}
        {/* 正文内容 - 流式输出时显示原始文本+光标 */}
        <div className="flex justify-start">
          <div className="max-w-[85%] rounded-lg px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100 text-sm">
  ...
  if (message.loading) {
    return (
      <div className="flex justify-start">
        <div className="max-w-[85%] rounded-lg px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100">
  ...
  if (message.error || message.status === 'error') {
    return (
      <div className="flex flex-col gap-1">
        ...
        {/* 错误提示 */}
        <div className="flex justify-start">
          <div className="max-w-[85%] rounded-lg px-3 py-2 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-sm">
      ...
      <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
        ...
        >
          {isUser ? (
            ...
            <MarkdownRenderer content={message.content || (message.status === 'done' ? '（无内容）' : '')} />
          )}
        </div>
      </div>
    </div>
