...
export class TrapezoidRenderer implements ShapeRenderer {
  readonly shape = "trapezoid";
  ...
  renderNode(ctx: CanvasRenderingContext2D, node: NodeData, _graph: Graph): void {
    ...
    const topWidth = width * 0.5; // 上边宽度
    const topOffset = (width - topWidth) / 2;

    ctx.save();
    ctx.beginPath();
    ctx.moveTo(x + topOffset, y); // 左上角
    ctx.lineTo(x + topOffset + topWidth, y); // 右上角
